...
export default function CheckoutPaymentStep() {
    ...
    useEffect(() => {
        ...
        } else if (!cart.orderReceipt || !cart.orderReceipt.date) {
        ...
        <>
            <form className="form-mode-paiement-tunnel" onSubmit={onSubmitPayment}>
                <div className="columns-picker-paiement-tunnel w-row">
                    {
                        ...
                        )) : <p>{t('modules/pointofsale-aquila:checkoutPaymentStep.noPayment')}</p>
                    }
                </div>
                ...
                <Link href="/checkout/delivery">
                    <a className="log-button-03 w-button">{t('modules/pointofsale-aquila:checkoutPaymentStep.previous')}</a>
                </Link>
                &nbsp;
                {
                    paymentMethods.length > 0 && (
                        <Button 
                            text={t('modules/pointofsale-aquila:checkoutPaymentStep.pay')}
                            loadingText={t('modules/pointofsale-aquila:checkoutPaymentStep.submitLoading')}
                            isLoading={isLoading}
