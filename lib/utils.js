...
// Check if all products are virtual in cart
export const isAllVirtualProducts = (cart) => {
    return cart.items.filter((item) => !item.typeDisplay).every((item) => item.type.startsWith('virtual'));
};
