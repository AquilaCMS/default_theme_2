...
export const convertFilter = (filter, lang) => {
    ...
    if (rawFilter.conditions.price) {
        newFilter.$and.push({ $or: [{ 'price.ati.normal': { $gte: rawFilter.conditions.price.min, $lte: rawFilter.conditions.price.max } }, { 'price.ati.special': { $gte: rawFilter.conditions.price.min, $lte: rawFilter.conditions.price.max } }] });
        delete rawFilter.conditions.price;
    }

    // Attribute filter management
    if (rawFilter.conditions.attributes) {
        for (const [attributeId, values] of Object.entries(rawFilter.conditions.attributes)) {
            newFilter.$and.push({ attributes: { $elemMatch: { [`translation.${lang}.value`]: { $in: values }, id: attributeId } } });
        }
        delete rawFilter.conditions.attributes;
    }

    // Picto filter management
    if (rawFilter.conditions.pictos) {
        newFilter.$and.push({ pictos: { $elemMatch: { code: { $in: rawFilter.conditions.pictos } } } });
        delete rawFilter.conditions.pictos;
    }
...
export const filterFix = (filter, priceEnd) => {
    if (filter.conditions?.price) {
        if (filter.conditions.price.min < priceEnd.min) {
            filter.conditions.price.min = priceEnd.min;
        }
        if (filter.conditions.price.max > priceEnd.max) {
            filter.conditions.price.max = priceEnd.max;
        }
        if (filter.conditions.price.min === priceEnd.min && filter.conditions.price.max === priceEnd.max) {
