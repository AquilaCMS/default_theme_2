...
export const formatStock = (stock) => {
    ...
    return <span style={{ fontWeight: 'bold', color }} itemProp="availability" itemType={`http://schema.org/${getAvailability(stock)}`}>{stock.value.replace('{date}', stock.date_supply ? formatDate(stock.date_supply) : '?')}</span>;
};
