...
export default function CheckoutAddressStep({ user }) {
    ...
    const onSubmitAddress = async (e) => {
        ...
        try {
            ...
            // Set cart addresses
            const newCart = await setCartAddresses(cart._id, { billing: addresses[0], delivery: {} });
            setCart(newCart);
        ...
        <>
            <form className="form-mode-paiement-tunnel" onSubmit={onSubmitAddress}>
                ...
                <Link href="/checkout/cart">
                    <a className="log-button-03 w-button">{t('modules/pointofsale-aquila:checkoutAddressStep.previous')}</a>
                </Link>
                &nbsp;
                <Button 
