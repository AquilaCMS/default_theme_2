...
export const getBodyRequestProductsFromCookie = (cookiesServerInstance) => {
    let cookieBody = '';
    if (cookiesServerInstance) {
        cookieBody = decodeURIComponent(cookiesServerInstance.get('bodyRequestProducts'));
    } else {
        cookieBody = cookie.parse(document.cookie).bodyRequestProducts; // "parse" function use already decodeURIComponent (see https://github.com/jshttp/cookie)
    }
    let body = {};
    if (cookieBody) {
        try {
...
export const convertFilter = (filter, lang) => {
    ...
    if (rawFilter.$text) {
        newFilter.$text = rawFilter.$text;
        delete rawFilter.$text;
    }

    // Price filter management
    if (rawFilter.price) {
        newFilter.$and.push({ $or: [{ 'price.ati.normal': { $gte: rawFilter.price.min, $lte: rawFilter.price.max } }, { 'price.ati.special': { $gte: rawFilter.price.min, $lte: rawFilter.price.max } }] });
